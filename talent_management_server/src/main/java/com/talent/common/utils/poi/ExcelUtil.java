...
import com.talent.common.utils.exception.UtilException;
import com.talent.common.utils.file.FileTypeUtils;
import com.talent.common.utils.file.FileUtils;
import com.talent.common.utils.file.ImageUtils;
import com.talent.common.utils.reflect.ReflectUtils;
import com.talent.common.utils.text.Convert;
import com.talent.system.config.SystemConfig;
import com.talent.system.config.annotation.Excel;
import com.talent.system.config.annotation.Excels;
...
public class ExcelUtil<T>
{
    ...
    public void setCellVo(Object value, Excel attr, Cell cell)
    {
        ...
        else if (Excel.ColumnType.PERCENT == attr.cellType())
        {
            if (StringUtils.isNotNull(value))
            {
                cell.setCellValue(StringUtils.fmtPct(value));
            }
        }
...
//        return DictUtils.getDictValue(dictType, dictLabel, separator);
     ...
     * @param fontSize     字号（如：11、12、16）
     * @param bold         是否加粗字体
     * @param hAlign       水平对齐方式（如：居中、靠左）
     * @param vAlign       垂直对齐方式（如：垂直居中）
     * @param hasFill      是否启用背景填充
     * @param fillColor    背景颜色（可用 new Color(r,g,b)，如 new Color(0xFF,0xC0,0x00)）
     * @param hasBorder    是否设置边框（全边框）
     * @return 设置好的 CellStyle 对象
     */
    public static CellStyle createCellStyle(Workbook workbook,
        ...
        font.setFontName(fontName);                  // 字体名：宋体、Arial 等
        font.setFontHeightInPoints((short) fontSize); // 字号：单位 pt
        font.setBold(bold);                          // 加粗
        style.setFont(font);

        // 对齐设置
        style.setAlignment(hAlign);                  // 水平对齐
        style.setVerticalAlignment(vAlign);          // 垂直对齐

        // 背景填充颜色
        if (hasFill && fillColor != null) {
            XSSFColor xssfColor = new XSSFColor(fillColor, new DefaultIndexedColorMap());
    ...
    // 缓存每个 workbook 对应的数值样式，避免重复创建
    private static final Map<Workbook, CellStyle> NUMERIC_STYLE_CACHE = new ConcurrentHashMap<>();

    /**
     * 获取缓存的数值单元格样式（千分位，两位小数，右对齐）
     */
    public static CellStyle getCachedNumberStyle(Workbook workbook) {
    ...
    /**
     * 创建带格式的数值单元格（支持 BigDecimal 和 Double）
     */
    public static void createNumericCell(Row row, int colIndex, Object value, Workbook workbook) {
        Cell cell = row.createCell(colIndex);

        if (value != null) {
            double doubleVal;
            if (value instanceof BigDecimal) {
                doubleVal = ((BigDecimal) value).doubleValue();
            } else if (value instanceof Number) {
                doubleVal = ((Number) value).doubleValue();
            } else {
                throw new IllegalArgumentException("只支持 BigDecimal 或 Number 类型的数值");
            }
        ...
        // 使用缓存样式
        cell.setCellStyle(getCachedNumberStyle(workbook));
    }
